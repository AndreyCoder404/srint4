...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	spentCalories := ((walkingCaloriesWeightMultiplier*weight + math.Sqrt((meanSpeed(action, duration)/3.6))/(height/100)) * walkingSpeedHeightMultiplier * weight) * duration * minInH
	return spentCalories
}
